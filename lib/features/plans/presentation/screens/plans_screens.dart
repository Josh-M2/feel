  ...
  Widget _buildBody(
    ...
    if (snapshot.hasError) {
      return const _PlansMessageBody(
        ...
        message:
            'Reading plans could not be loaded right now. Please try again in a moment.',
    ...
    if (data.plans.isEmpty) {
      return const _PlansMessageBody(
        ...
        message: 'Reading plans will appear here when they are ready.',
        icon: Icons.menu_book_outlined,
      );
    }

    final ReadingPlan continuePlan = data.continuePlan ?? data.plans.first;
    final PlanDay continueDay =
        data.currentDay ?? continuePlan.resolvedCurrentDay;
  ...
  Widget _buildBody(
    ...
    if (snapshot.hasError) {
      return const _PlansMessageBody(
        ...
        message:
            'This plan could not be loaded right now. Please try again in a moment.',
        icon: Icons.cloud_off_rounded,
      );
    }
...
class PlanDayReadScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<PlanDayReadState>(
      future: _loadScreenData(),
      builder:
          (BuildContext context, AsyncSnapshot<PlanDayReadState> snapshot) {
            return TabScreenScaffold(
              title: 'Day read',
              subtitle: 'Plans',
              showBackButton: true,
  ...
  Widget _buildBody(
    ...
    if (snapshot.hasError || !snapshot.hasData) {
      return const _PlansMessageBody(
        ...
        message:
            'This reading day could not be loaded right now. Please try again in a moment.',
        icon: Icons.cloud_off_rounded,
      );
    }
  ...
  Widget _buildBody(
    ...
    if (snapshot.connectionState == ConnectionState.waiting) {
      return const _PlansLoadingBody(
        ...
        subtitle:
            'Collecting your plan snapshots and overall reading progress.',
      );
    }
    ...
    if (snapshot.hasError) {
      return const _PlansMessageBody(
        ...
        message:
            'Plan progress could not be loaded right now. Please try again in a moment.',
        icon: Icons.cloud_off_rounded,
      );
    }
...
class _PlansLoadingBody extends StatelessWidget {
  const _PlansLoadingBody({required this.title, required this.subtitle});
...
class _PlanDetailScreenData {
  const _PlanDetailScreenData({required this.plan, required this.currentDay});
