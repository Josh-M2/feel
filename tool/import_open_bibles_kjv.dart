...
Future<void> main(List<String> arguments) async {
  final ImportOptions options = parseArguments(arguments);
...
ImportOptions parseArguments(List<String> arguments) {
  String? sourcePath;
  String? outputPath;
...
void _printUsage() {
  stdout.writeln(
    'Usage: dart run tool/import_open_bibles_kjv.dart '
    '--source <eng-kjv.osis.xml> --out <output.sql>',
  );
}
...
ImportDocument parseOpenBiblesKjvOsis(
  ...
  final List<ParsedVerse> verses = parser.parse();
...
String buildImportSql(ImportDocument document) {
  final StringBuffer buffer = StringBuffer()
    ..writeln('-- Generated by tool/import_open_bibles_kjv.dart')
    ..writeln('-- Source: Open Bibles KJV OSIS XML')
    ..writeln('begin;')
    ..writeln()
    ..writeln(
      'insert into public.content_bible_versions (id, name, language_code, is_default, is_public_domain, is_active)',
    )
    ..writeln("values ('kjv', 'King James Version', 'en', true, true, true)")
  ...
  _writeVerseImport(buffer, document.verses);
  ...
  buffer
    ...
    ..writeln('-- select * from public.validate_kjv_scripture_import();');
...
void _writeVerseImport(StringBuffer buffer, List<ParsedVerse> verses) {
  buffer.writeln(
    "delete from public.content_bible_verses where version_id = 'kjv';",
  );
  buffer.writeln();
  ...
  for (int start = 0; start < verses.length; start += batchSize) {
    ...
    for (int index = 0; index < batch.length; index++) {
      ...
      final String suffix = index == batch.length - 1 ? '' : ',';
      buffer.writeln(
        "  ('kjv', ${_sql(verse.book.id)}, ${verse.chapterNumber}, ${verse.verseNumber}, ${_sql(verse.text)})$suffix",
      );
    }
    buffer.writeln(';');
    buffer.writeln();
